...
import com.yunchen.maidsoulcore.forge.network.VisionCaptureRequestPacket;
import com.yunchen.maidsoulcore.forge.network.VisionCaptureResultPacket;
import com.yunchen.maidsoulcore.forge.network.VisionProxyImagePacket;
...
public final class VisionCaptureClient {
    ...
    public static void captureAndSend(VisionCaptureRequestPacket request) {
        ...
        try (NativeImage nativeImage = new NativeImage(
            ...
            MaidSoulCoreMod.LOGGER.warn("MaidSoulCore client vision capture failed", e);
        } catch (Exception e) {
            MaidSoulCoreMod.LOGGER.warn("MaidSoulCore client vision capture failed", e);
        }
    }
    ...
    private static void sendClientDirectSummary(VisionCaptureRequestPacket request, VisionConfig config, String imageBase64) {
        try {
            ...
            MaidSoulCoreMod.LOGGER.warn("MaidSoulCore client vision summary failed", e);
            ModNetwork.CHANNEL.sendToServer(new VisionCaptureResultPacket(
                    request.maidUuid(),
                    request.requestId(),
                    request.reason(),
