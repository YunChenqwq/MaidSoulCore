...
public final class MaidSoulActionCatalogService {
    ...
    public static String actionTypeSummary() {
        return """
                ...
                ENTER_COMBAT_GROUP: 进入战斗任务并按过滤条件逐个清理，action_value 支持 type/tag，例如 type=minecraft:zombie 或 #forge:skeletons
                """.trim();
    }
    ...
    public static String builtInSkillSummary() {
        return """
                ...
                - action_value 支持 type/tag 过滤，例如 type=minecraft:zombie、tag=forge:undead、type=minecraft:zombie;tag=forge:undead、#forge:skeletons
                - 群体模式会在合法目标中优先选择最近目标逐个处理；若指定目标太远，不会强行先追远处目标
