...
public final class MaidSoulRuntimeClient implements LLMClient {
    ...
    public void chat(LLMCallback callback) {
        ...
        try {
            ...
            MaidSoulCoreForgeMod.LOGGER.info("MaidSoulRuntimeClient.chat called. maid={} llmSite={} llmModel={} availableRuntimeSite={} messages={}",
                    maid.getUUID(),
            ...
            if (!latestUserMessage.isBlank()) {
                ...
                MaidSoulCoreForgeMod.LOGGER.warn("MaidSoul runtime received an LLM callback without a real owner message. maid={}", maid.getUUID());
                MaidSpeechDispatcher.queueSpeechOnServer(maid, "我听到了调用，但没读到真正的主人消息。检查一下 TLM 的模型站点配置。");
            }
        } catch (Exception e) {
            MaidSoulCoreForgeMod.LOGGER.error("MaidSoul runtime bridge failed. maid={}", maid.getUUID(), e);
    ...
    private static void installMaidSoulSentinelSetting(LLMCallback callback) {
        ...
        if (maid.level() instanceof ServerLevel serverLevel) {
            serverLevel.getServer().submit(() -> maid.getChatBubbleManager().addLLMChatText("", callback.getWaitingChatBubbleId()));
        }
        MaidSoulCoreForgeMod.LOGGER.info("Installed MaidSoulCore sentinel setting for maid {}, bypassing TLM auto setting generation.", maid.getUUID());
    }
