...
public final class ProactiveScheduler {
    ...
    public boolean shouldScheduleAfterSilentDecision(int activeCuriosity, int silentDecisions) {
        if (silentDecisions <= 0) {
            return true;
        }
        // 连续沉默判断后，只有主动好奇足够高才继续排候选；这仍然只是唤醒 planner，不代表一定回复。
        return activeCuriosity >= 65 && silentDecisions < 3;
    }
    ...
    public int nextDelaySeconds(int stage, int activeCuriosity, int silentDecisions) {
    ...
    public String buildEvent(long silentSeconds, int stage, int activeCuriosity, String affectHint) {
        ...
        return "[主动候选事件] 上一轮可见发言结束后，玩家沉默约 " + silentSeconds + " 秒。"
                + "阶段=" + stageName(stage) + "。"
